...
module.exports = class extends Command {
	...
	formatCommand(message, prefix, richDisplay, command) {
		...
		return richDisplay ? `**\`${prefix}${command.name}\`** → *${description}*` : `${prefix}${command.name.padEnd([...this.client.commands.keys()].reduce((long, str) => Math.max(long, str.length), 0))} ::\t${description}`;
	}
